...
export function DarkBreakSection() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      [[0, 1], [1, 2], [1, 4], [3, 4], [4, 5]].forEach(([i, j]) => {
      ...
      nodes.forEach((node) => {
        const x = node.x * w;
        const y = node.y * h;
        const pulse = node.status === 'error' 
          ? Math.sin(time * 3) * 0.3 + 1.3
          : 1;
  ...
  const pipelineSteps = [
    { 
      icon: GitCommit, 
      label: 'Git Push', 
      description: 'Commit triggers webhook',
      status: 'complete',
    ...
    { 
      icon: TestTube2, 
      label: 'Fortress Test Loop', 
      description: 'Comprehensive QA execution',
      status: 'active',
      color: 'var(--accent-blue)'
    },
    { 
      icon: EyeIcon, 
      label: 'Visual Cortex Update', 
    ...
    { 
      icon: CheckCircle, 
      label: 'Merge with Confidence', 
      description: 'Autonomous approval',
      status: 'pending',
      color: 'rgba(255, 255, 255, 0.3)'
    }
  ];
  ...
  return (
    ...
    >
      <div className="max-w-[1200px] mx-auto">
        ...
        {/* Large Centered Pipeline Visual */}
        <motion.div
        ...
        >
          <div 
            className="max-w-[700px] mx-auto rounded-[20px] border p-10"
            style={{ 
          ...
          {features.map((feature, index) => (
            ...
            >
              <div 
                ...
                style={{ backgroundColor: 'rgba(47, 182, 124, 0.15)' }}
              >
                <feature.icon 
                  className="w-5 h-5" 
