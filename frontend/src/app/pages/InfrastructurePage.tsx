...
import { ChevronDown, Download, RefreshCw, Shield, Lock, Zap, RotateCcw, DollarSign, TrendingDown, Server, Database, Activity, CloudCog, Home, Folder, Sun, Moon } from 'lucide-react';
      ...
      <div className="w-full h-screen flex flex-col overflow-hidden bg-zinc-50 dark:bg-[#010308] font-['Geist_Sans',_'Inter',_sans-serif] transition-colors duration-300 relative">
        ...
        <div className="flex-1 flex flex-row overflow-hidden relative z-10">
          ...
          >
            ...
            {/* Panel Header — sticky */}
            <div className="sticky top-0 z-20 px-5 py-3 border-b border-gray-200 dark:border-slate-800 flex items-center justify-between bg-zinc-50/80 dark:bg-slate-900/60 backdrop-blur-sm transition-colors">
              <div className="flex items-center gap-3">
                <CloudCog className="w-4 h-4 text-indigo-500 dark:text-indigo-400" />
                <span className="font-['JetBrains_Mono',_monospace] text-[13px] font-semibold text-zinc-800 dark:text-slate-200">
                  Generated Infrastructure
                </span>
              </div>
              <div className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400 border border-indigo-100/50 dark:border-indigo-800/30 flex items-center gap-1.5 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto bg-transparent relative z-10 custom-scrollbar">
              <pre className="text-[13px] leading-relaxed py-3 font-['JetBrains_Mono',_monospace]">
                {terraformCode.split('\n').map((line, index) => {
                  ...
                  if (isComment) {
                    highlightedLine = <span className="text-zinc-500 dark:text-slate-500 transition-colors">{line}</span>;
                  } else if (isAnnotation) {
                    highlightedLine = <span className="text-indigo-600 dark:text-indigo-400 font-semibold transition-colors">{line}</span>;
                  } else {
                      ...
                      <>
                        {parts.map((part, i) => {
                          ...
                          if (part.startsWith('"') && part.endsWith('"')) {
                            ...
                            return <span key={i} className="text-pink-600 dark:text-pink-400 font-medium">{part}</span>;
                            ...
                            return <span key={i} className="text-sky-600 dark:text-sky-400 font-medium">{part}</span>;
                  ...
                  return (
                    <div
                      key={index}
                      className={`flex items-start hover:bg-zinc-100/50 dark:hover:bg-slate-800/50 transition-colors ${isAddedLine ? 'border-l-[3px] border-l-emerald-500' : 'border-l-[3px] border-l-transparent'}`}
                    >
                      <span
                        className={`inline-block w-10 text-right pr-4 mr-0 select-none flex-shrink-0 text-zinc-400 dark:text-slate-500 border-r border-zinc-200 dark:border-slate-800 text-[11px] pt-0.5 font-mono transition-colors pl-2 ${isAddedLine ? 'bg-emerald-50/30 dark:bg-emerald-900/10' : ''}`}
                      >
                        {lineNumber}
                      </span>
                      <div className={`flex-1 break-all pl-4 ${isAddedLine ? 'bg-emerald-50/60 dark:bg-emerald-900/20' : ''}`}>
                        {highlightedLine}
                      </div>
                    </div>
                  );
